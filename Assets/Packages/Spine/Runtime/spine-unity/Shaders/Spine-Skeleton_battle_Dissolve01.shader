Shader "Spine/Skeleton_battle_Dissolve01"
{
    ...
    // {
        Properties
        {
            ...
            [Space]
            [Header(Dissolve)]
            _Dissolve ("Dissolve", vector) = (1, 2, 0, 0)
            // _DisAmount ("DisAmount", Range(0, 2)) = 2
            // _DissolveColorWidth ("DissolveColorWidth", Range(0, 0.1)) = 0
        ...
        SubShader
        {
            ...
            Pass
            {
                ...
                {
                    float4 rawColor = tex2D(_MainTex, i.uv) * _Color ;//采样主贴图
                    float3 zn = tex2D(_Noise, i.worldPosition.xy * _Dissolve.r); // 采样溶解贴图并让其有个缩放效果
                    // 乘上noise贴图
                    float MovingPosOnModel = _Dissolve.g * zn.r;
                    
                    // 发光长度
                    float maintexturePart = MovingPosOnModel - _Dissolve.b;
                    
                    maintexturePart = step(_Dissolve.a, maintexturePart);
                    // return maintexturePart;
                    // normal贴图部分
        ...
        Pass
        {
            ...
            fixed4 Frag(v2f i): SV_Target
            {
                float4 rawColor = tex2D(_MainTex, i.uv) ;//采样主贴图
                float3 zn = tex2D(_Noise, i.worldPosition.xy * _Dissolve.r); // 采样溶解贴图并让其有个缩放效果
                // 乘上noise贴图
                float MovingPosOnModel = _Dissolve.g * zn.r;
    ...
    CustomEditor"ShaderGUI_Skeleton_battle_Dissolve01"
}
