...
public class TimeManager : ManagerBase<TimeManager>, ManagerInterface
{
    ...
    public static TimeManager Instance
    {
        get
        {
            return AppFacade.Instance.GetManager<TimeManager>(ManagerName.Timer); ;
        }
    }
    ...
    {
        MyNetWorkManager.Instance.SendGetMessage(URL.TIMEURL, null, (string res) =>
        {
            try
            {
                if (!string.IsNullOrEmpty(res))
                {
                    JSONObject data = JSON.Parse(res).AsObject;
            ...
            {
                MyException.AddException("获取时间戳报错:", e);
            }
        });
    }
    /// <summary>
    ...
    public DateTime Now
    {
        get
        {
            if (networkTimestamp == 0)
            {
                MyException.AddException("未同步网络时间却想要取得网络时间！");
                return DateTime.Now;
            }
